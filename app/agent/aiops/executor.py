...
def _handle_tool_error(e: Exception) -> str:
    """自定义工具错误处理器，捕获所有异常包括 ToolException"""
    logger.warning(f"工具调用异常: {type(e).__name__}: {e}")
...
async def executor(state: PlanExecuteState) -> Dict[str, Any]:
    ...
    try:
        ...
        # 创建工具节点，使用自定义错误处理器捕获所有异常（包括 ToolException）
        tool_node = ToolNode(all_tools, handle_tool_errors=_handle_tool_error)
...
- 专注于当前步骤，不要考虑其他任务"""),
        ...
        if hasattr(llm_response, "tool_calls") and llm_response.tool_calls:
            ...
            # 处理 ToolNode 可能返回的不同格式
            if isinstance(tool_messages, dict):
                tool_result_messages = tool_messages.get("messages", [])
            elif isinstance(tool_messages, list):
                tool_result_messages = tool_messages
            else:
            ...
            content = llm_response.content if hasattr(llm_response, 'content') else None
            result = content if isinstance(content, str) else str(content) if content else "LLM 未返回内容"
